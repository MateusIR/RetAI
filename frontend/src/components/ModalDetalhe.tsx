              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-surface-3 rounded-lg p-3 print:bg-gray-100 print:text-black">
                  ...
                  <p className={`text-sm font-medium ${
                    ...
                    data.status === 'PROCESSANDO' ? 'text-amber-400 print:text-amber-700' : 'text-red-400 print:text-red-700'
                  }`}>
              ...
              {(mostrarImagens || document.documentElement.classList.contains('printing')) && data.imagens && (
                <div className="grid grid-cols-2 gap-4 mb-4">
                  {data.imagens.map((img, i) => (
                    <div key={i} className="rounded-lg overflow-hidden border border-surface-4 bg-black/50 print:border-gray-300">
                      {/* Nota: Ajuste a base URL conforme a sua variável de ambiente VITE_API_URL */}
                      <img 
                        src={`http://localhost:8000/imagens/${img.caminho.split('/').pop()}`} 
                        alt={`Olho ${img.tipo}`} 
                        className="w-full h-32 object-cover" 
                      />
                      <div className="bg-surface-3 p-2 text-center text-xs font-semibold text-slate-300 print:bg-gray-100 print:text-black">
                        Olho {img.tipo === 'OD' ? 'Direito (OD)' : 'Esquerdo (OE)'}
                      </div>
                    </div>
                  ))}
                </div>
