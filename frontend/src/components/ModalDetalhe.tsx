...
export function usePlatformDialog() {
  ...
  const showAlert = (title: string, message: string, type: DialogProps['type'] = 'info'): Promise<void> =>
    new Promise(resolve =>
      setDialog({ title, message, type, confirmLabel: 'Ok', onConfirm: () => { setDialog(null); resolve(true as any) }, resolve: resolve as any })
    )

  const showConfirm = (title: string, message: string, type: DialogProps['type'] = 'warning', confirmLabel = 'Confirmar', cancelLabel = 'Cancelar'): Promise<boolean> =>
    new Promise(resolve =>
      setDialog({
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-surface-3 rounded-lg p-3 print:bg-gray-100 print:text-black">
                    ...
                    <p className={`text-sm font-medium ${
                      data.status === 'CONCLUIDO'   ? 'text-emerald-400 print:text-emerald-700' :
                      data.status === 'PROCESSANDO' ? 'text-amber-400 print:text-amber-700'     :
                ...
                {data.status === 'CONCLUIDO' && data.imagens && data.imagens.length > 0 && (
                  <div className="flex items-center gap-2 mb-2 print:hidden">
                    <input
                      type="checkbox" id="toggle-imagens"
                      className="w-4 h-4 accent-accent cursor-pointer"
                      checked={mostrarImagens}
                      onChange={e => setMostrarImagens(e.target.checked)}
                    />
                    <label htmlFor="toggle-imagens" className="text-sm font-medium text-slate-300 cursor-pointer select-none">
