...
function ImagemLightbox({ src, label, onClose }: { src: string; label: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col items-center justify-center bg-black/90 backdrop-blur-md animate-fade-in" onClick={onClose}>
      <div className="relative max-w-3xl w-full mx-4 flex flex-col items-center gap-3" onClick={e => e.stopPropagation()}>
        <img src={src} alt={label} className="w-full max-h-[75vh] object-contain rounded-xl shadow-2xl border border-white/10" />
        <p className="text-sm font-semibold text-slate-300 tracking-wide">{label}</p>
        <button onClick={onClose} className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-surface-3 border border-surface-4 flex items-center justify-center text-slate-400 hover:text-white hover:bg-surface-4 transition-colors">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
      </div>
      <p className="mt-4 text-xs text-slate-600">{useTranslation().t('app.clickOrEsc')}</p>
    </div>
  )
}
...
export default function ModalDetalhe({ diagnosticoId, onClose, currentUser }: Props) {
  ...
  useEffect(() => {
    fetchDiagnostico(diagnosticoId)
      .then((d) => { setData(d); setParecer(d.parecer || ''); })
      .finally(() => setLoading(false))
  }, [diagnosticoId])

  useEffect(() => {
    if (!data || !podeEditarParecer) return
    const timer = setTimeout(() => { atualizarParecer(data.id, parecer).catch(console.error) }, 800)
  ...
  return (
    <>
      {imagemExpandida && <ImagemLightbox src={imagemExpandida.src} label={imagemExpandida.label} onClose={() => setImagemExpandida(null)} />}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm animate-fade-in print:bg-transparent print:backdrop-blur-none" onClick={e => e.target === e.currentTarget && onClose()}>
        <div className="card w-full max-w-lg mx-4 animate-slide-up overflow-hidden max-h-[90vh] flex flex-col modal-print-area print:max-h-none print:shadow-none print:border-none">
          <div className="flex items-center justify-between px-6 py-4 border-b border-surface-4 print:border-b-black">
            ...
            <div className="flex gap-2 print:hidden">
              {podeGerarPDF ? (
                ...
                <span className="text-xs px-3 py-1.5 h-auto flex items-center gap-2 text-slate-600 cursor-not-allowed" title={t('app.verifiedOnlyPDF')}>
          ...
          <div className="overflow-y-auto flex-1 p-6 print:overflow-visible">
            {loading ? (
              <div className="space-y-4 print:hidden">
                {[1, 2, 3].map(i => <div key={i} className="shimmer-line h-6 rounded" style={{ width: `${70 + i * 10}%` }} />)}
              </div>
            ) : data ? (
              <div className="space-y-5">
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-surface-3 rounded-lg p-3 print:bg-gray-100 print:text-black">
                    ...
                    <p className="text-sm font-medium text-slate-200 print:text-black">{data.idade} · {data.sexo === 'M' ? t('app.male') : t('app.female')}</p>
                  </div>
                  <div className="bg-surface-3 rounded-lg p-3 print:bg-gray-100 print:text-black">
                    ...
                    <p className={`text-sm font-medium ${data.status === 'CONCLUIDO' ? 'text-emerald-400 print:text-emerald-700' : data.status === 'PROCESSANDO' ? 'text-amber-400 print:text-amber-700' : 'text-red-400 print:text-red-700'}`}>
                      {data.status === 'CONCLUIDO' ? t('app.concluded') : data.status === 'PROCESSANDO' ? t('app.processing') : t('app.error')}
                    </p>
                  </div>
                </div>
                ...
                {data.status === 'CONCLUIDO' && data.imagens && data.imagens.length > 0 && (
                  <div className="flex items-center gap-2 mb-2 print:hidden">
                    ...
                    <label htmlFor="toggle-imagens" className="text-sm font-medium text-slate-300 cursor-pointer select-none">{t('app.toggleImages')}</label>
                  </div>
                )}
                ...
                {(mostrarImagens || document.documentElement.classList.contains('printing')) && data.imagens && (
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    {data.imagens.map((img, i) => {
                      ...
                      return (
                        <div key={i} className="rounded-lg overflow-hidden border border-surface-4 bg-black/50 print:border-gray-300 cursor-zoom-in group relative" onClick={() => setImagemExpandida({ src, label })}>
                          <img src={src} alt={label} className="w-full h-32 object-cover transition-transform duration-300 group-hover:scale-105" />
                          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors duration-200 flex items-center justify-center print:hidden">
                            <svg className="w-7 h-7 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200 drop-shadow-lg" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-4.35-4.35M11 19a8 8 0 100-16 8 8 0 000 16zM11 8v6M8 11h6" /></svg>
                          </div>
                ...
                {data.status === 'CONCLUIDO' && gruposPorOlho && Object.keys(gruposPorOlho).length > 0 ? (
                  Object.entries(gruposPorOlho).map(([olho, resultados]) => (
                    <div key={olho}>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="text-sm font-semibold text-slate-200 print:text-black">{olho === 'OD' ? t('app.od') : t('app.oe')}</h3>
                      </div>
                  ...
                  <div className="flex items-center gap-3 text-amber-400 text-sm bg-amber-500/10 border border-amber-500/20 rounded-lg p-4 print:hidden">
                    <svg className="w-5 h-5 animate-spin flex-shrink-0" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
                    {t('diagnosis.analysisInProgress')}
                  </div>
                ) : data.status === 'ERRO' ? (
                  <div className="text-red-400 text-sm bg-red-500/10 border border-red-500/20 rounded-lg p-4 print:hidden">{t('diagnosis.errorOccurred')}</div>
                ) : null}

                {data.status === 'CONCLUIDO' && (
                  <div className="pt-4 border-t border-surface-4 print:border-black">
                    <label className="text-xs text-slate-500 block mb-1 print:text-gray-600">{t('app.report')}</label>
                    {podeEditarParecer ? (
                      <textarea className="w-full bg-surface-3 border border-surface-4 rounded-lg p-3 text-sm text-slate-200 placeholder-slate-600 resize-none print:bg-white print:text-black print:border-black" rows={3} placeholder={t('app.reportPlaceholder')} value={parecer} onChange={e => setParecer(e.target.value)} />
                    ) : (
                      <div className="relative group">
                        <textarea className="w-full bg-surface-3/40 border border-surface-4/50 rounded-lg p-3 text-sm text-slate-600 placeholder-slate-700 resize-none cursor-not-allowed" rows={3} placeholder={t('app.reportPlaceholder')} disabled />
          ...
          <div className="px-6 py-4 border-t border-surface-4 print:hidden">
            <button onClick={onClose} className="btn-ghost w-full">{t('app.closeModal')}</button>
          </div>
        </div>
      </div>
    </>
