        ...
        <div className="overflow-y-auto flex-1 p-6 print:overflow-visible">
          {loading ? (
            ...
            <div className="space-y-5">
              ...
              {/* CORREÇÃO DO CAMINHO DA IMAGEM AQUI */}
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  {data.imagens.map((img, i) => (
                    <div key={i} className="rounded-lg overflow-hidden border border-surface-4 bg-black/50 print:border-gray-300">
                      <img src={`http://localhost:8000/imagens_salvas/${img.caminho.split(/[/\\]/).pop()}`} alt={`Olho ${img.tipo}`} className="w-full h-32 object-cover" />
                      <div className="bg-surface-3 p-2 text-center text-xs font-semibold text-slate-300 print:bg-gray-100 print:text-black">Olho {img.tipo === 'OD' ? 'Direito (OD)' : 'Esquerdo (OE)'}</div>
                    </div>
                  ))}
                </div>
              )}
