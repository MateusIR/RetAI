...
export default function TelaNovo({ onSuccess, onNovo }: Props) {
  ...
  const podeEnviar =
    ...
    form.idade &&
    Number(form.idade) > 0 &&
    ((form.od && form.fileOd) || (form.oe && form.fileOe))
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-slide-up">
        ...
        <p className="text-slate-400 text-sm mb-1">
          O diagnóstico #{diagnosticoId} está em fila de processamento.
        </p>
        <p className="text-slate-500 text-xs mb-10">
          Acompanhe o status na aba Diagnósticos.
        </p>
        <div className="flex gap-3">
          <button onClick={onSuccess} className="btn-primary flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" />
            </svg>
            Ver diagnósticos
          </button>
          <button onClick={handleNovo} className="btn-ghost flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="max-w-xl animate-fade-in">
      <div className="card p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select
                  className="input-field"
                  value={form.sexo}
                  onChange={e => setField('sexo', e.target.value as 'M' | 'F')}
                >
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="flex items-center gap-3 mb-3 cursor-pointer group">
                <div
                  className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors
                    ${form.od ? 'bg-accent border-accent' : 'border-surface-4 group-hover:border-accent/50'}`}
                  onClick={() => setField('od', !form.od)}
                >
                  {form.od && (
              ...
              {form.od && (
                <div className="animate-slide-up">
                  <FileDropZone
                    label="Imagem do Olho Direito (fundoscopia / OCT)"
                    file={form.fileOd}
                    onChange={f => setField('fileOd', f)}
                  />
                </div>
              )}
            </div>
            ...
            <div>
              <label className="flex items-center gap-3 mb-3 cursor-pointer group">
                <div
                  className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors
                    ${form.oe ? 'bg-accent border-accent' : 'border-surface-4 group-hover:border-accent/50'}`}
                  onClick={() => setField('oe', !form.oe)}
                >
                  {form.oe && (
                    <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
              ...
              {form.oe && (
                <div className="animate-slide-up">
                  <FileDropZone
                    label="Imagem do Olho Esquerdo (fundoscopia / OCT)"
                    file={form.fileOe}
                    onChange={f => setField('fileOe', f)}
                  />
                </div>
              )}
            </div>
        ...
        <button
          ...
          disabled={!podeEnviar || step === 'sending'}
        ...
        {/* Hint de validação */}
        {!podeEnviar && (form.nome || form.idade || form.od || form.oe) && (
          <p className="text-xs text-slate-500 text-center -mt-2">
            Preencha nome, idade e envie ao menos uma imagem.
          </p>
        )}
      </div>
    </div>
