      ...
      <div className="card w-full max-w-md p-8 animate-fade-in shadow-2xl border-surface-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {(mode === "register" || mode === "admin_self_reset") && (
            <div className="animate-slide-up space-y-4">
              ...
              <div>
                ...
                <input required type="text" className="input-field" placeholder="Dr. Nome" maxLength={100} value={nome} onChange={(e) => setNome(e.target.value)} />
              </div>
          ...
          {mode !== "forgot" && (
            <div className="animate-fade-in">
              <div className="flex justify-between items-center">
                ...
                {mode === "login" && (
                  <button type="button" onClick={() => changeMode("forgot")} className="text-xs text-accent hover:text-teal-accent">
                    Esqueceu a senha?
                  </button>
                )}
              </div>
              <div className="relative mt-1">
                <input required type={showPassword ? "text" : "password"} className="input-field pr-10" placeholder="••••••••" maxLength={72} value={senha} onChange={(e) => setSenha(e.target.value)} />
              ...
              {(mode === "register" || mode === "admin_self_reset") && senha.length > 0 && (
          ...
          <button disabled={loading} type="submit" className="btn-primary w-full mt-6 py-3 relative overflow-hidden group">
            {loading ? "Processando..." : mode === "login" ? "Entrar no Sistema" : mode === "register" ? "Cadastrar Médico" : mode === "forgot" ? "Solicitar Redefinição" : "Redefinir Minha Senha"}
          </button>
        </form>
        ...
        <div className="mt-6 text-center border-t border-surface-4 pt-4 flex flex-col gap-2">
          ...
          <p className="text-sm text-slate-500">
            {mode === "login" ? "Não possui conta?" : mode === "register" ? "Já possui conta?" : "Lembrou a senha?"}
