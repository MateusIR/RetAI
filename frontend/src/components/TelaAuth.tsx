  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        setSuccessMsg("Conta não verificada criada! Você já pode fazer login.");
        changeMode("login");
      } else if (mode === "forgot") {
        ...
        setSuccessMsg("Solicitação enviada! Por favor, peça ao Administrador para aprovar e gerar uma nova senha no sistema.");
        ...
        setSuccessMsg("Sua senha de Administrador foi atualizada! Você já pode fazer login.");
        changeMode("login");
      }
    } catch (err: any) {
      ...
      {showUnverifiedWarning && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="card w-full max-w-sm p-6 text-center animate-slide-up border border-amber-500/30">
            ...
            <h2 className="text-lg font-bold text-white mb-2">Conta Não Verificada</h2>
            <div className="text-sm text-slate-400 mb-6 text-left space-y-2">
              <p>Atenção, contas não verificadas possuem limitações severas. Você <strong>NÃO</strong> poderá:</p>
              <ul className="list-disc pl-4 text-amber-400/80 space-y-1">
                <li>Ser promovido a Superadmin</li>
                <li>Gerar relatórios em PDF</li>
                <li>Preencher pareceres médicos</li>
                <li><strong>TODOS os seus diagnósticos serão apagados ao sair da sessão.</strong></li>
              </ul>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowUnverifiedWarning(false)} className="btn-ghost flex-1">Cancelar</button>
              <button 
      ...
      <div className="card w-full max-w-md p-8 animate-fade-in shadow-2xl border-surface-4">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1">
            ...
            {mode === "register_unverified" && "Cadastro Não Verificado"}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="animate-slide-up space-y-4">
              {mode === "admin_self_reset" && (
                <div className="bg-surface-3 p-3 rounded border border-surface-4 text-xs text-slate-400 text-center mb-4">
                  Para redefinir sua senha de Administrador localmente, você precisa confirmar os exatos dados do seu cadastro.
                </div>
              )}
              {mode === "register_unverified" && (
                <div className="bg-amber-500/10 p-3 rounded border border-amber-500/30 text-xs text-amber-400/90 text-center mb-4">
                  Conta com recursos limitados ativada.
                </div>
              )}
              
              <div>
                <label className="label">Nome Completo</label>
                <input required type="text" className="input-field" placeholder="Nome" maxLength={100} value={nome} onChange={(e) => setNome(e.target.value)} />
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input required type="text" className="input-field font-mono text-sm flex-1" placeholder="CRM" maxLength={10} value={crmNumero} onChange={(e) => setCrmNumero(e.target.value.replace(/\D/g, ""))} />
          ...
          <div>
            ...
            <input required type="email" className="input-field" placeholder="dr@clinica.com" maxLength={255} value={email} onChange={(e) => setEmail(e.target.value)} />
          </div>

          {mode !== "forgot" && (
            <div className="animate-fade-in">
              <div className="flex justify-between items-center">
                <label className="label mb-0">{mode === "admin_self_reset" ? "Nova Senha" : "Senha"}</label>
                {mode === "login" && (
                  <button type="button" onClick={() => changeMode("forgot")} className="text-xs text-accent hover:text-teal-accent">Esqueceu a senha?</button>
                )}
              </div>
              <div className="relative mt-1">
                <input required type={showPassword ? "text" : "password"} className="input-field pr-10 [&::-ms-reveal]:hidden [&::-webkit-credentials-auto-fill-button]:hidden" placeholder="••••••••" maxLength={72} value={senha} onChange={(e) => setSenha(e.target.value)} />
          ...
          <button disabled={loading} type="submit" className="btn-primary w-full mt-6 py-3 relative overflow-hidden group">
            {loading ? "Processando..." : mode === "login" ? "Entrar no Sistema" : mode === "register" || mode === "register_unverified" ? "Cadastrar Médico" : mode === "forgot" ? "Solicitar Redefinição" : "Redefinir Minha Senha"}
          </button>
          ...
          {mode === "register" && (
            <div className="mt-4 text-center">
              <button type="button" onClick={() => setShowUnverifiedWarning(true)} className="text-xs text-slate-500 hover:text-slate-300 underline underline-offset-2">
                Não possui CRM? Criar conta não verificada.
              </button>
            </div>
          )}
        </form>
        ...
        <div className="mt-6 text-center border-t border-surface-4 pt-4 flex flex-col gap-2">
          ...
          <p className="text-sm text-slate-500">
            {mode === "login" ? "Não possui conta?" : mode === "forgot" ? "Lembrou a senha?" : "Já possui conta?"}
