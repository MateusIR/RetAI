...
interface Props { onLogin: (token: string, user: any, expiresAt: number) => void; }
...
export default function TelaAuth({ onLogin }: Props) {
  ...
  const strengthText = strength === 0 ? "" : strength === 1 ? t("app.login.passwordStrength.weak") : strength === 2 ? t("app.login.passwordStrength.medium") : t("app.login.passwordStrength.strong");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!emailRegex.test(email.trim())) return setError(t("app.login.errors.invalidEmail"));
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="card w-full max-w-sm p-6 text-center animate-slide-up border border-amber-500/30">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => { setShowUnverifiedWarning(false); changeMode("register_unverified"); }} className="bg-amber-500 hover:bg-amber-600 text-white font-medium px-4 py-2 rounded-lg flex-1 transition-colors">
      ...
      <div className="card w-full max-w-md p-8 animate-fade-in shadow-2xl border-surface-4">
        ...
        {error && <div className="bg-red-500/10 border border-red-500/30 text-red-400 text-sm p-3 rounded-lg mb-6 text-center animate-fade-in">{error}</div>}
        {successMsg && <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm p-3 rounded-lg mb-6 text-center animate-fade-in">{successMsg}</div>}

        <form onSubmit={handleSubmit} className="space-y-4">
          {(mode === "register" || mode === "admin_self_reset" || mode === "register_unverified") && (
            <div className="animate-slide-up space-y-4">
              {mode === "admin_self_reset" && <div className="bg-surface-3 p-3 rounded border border-surface-4 text-xs text-slate-400 text-center mb-4">{t("app.login.adminResetTitle")}</div>}
              {mode === "register_unverified" && <div className="bg-amber-500/10 p-3 rounded border border-amber-500/30 text-xs text-amber-400/90 text-center mb-4">{t("app.login.unverifiedTitle")}</div>}
          ...
          {mode !== "forgot" && (
            <div className="animate-fade-in">
              <div className="flex justify-between items-center">
                ...
                {mode === "login" && <button type="button" onClick={() => changeMode("forgot")} className="text-xs text-accent hover:text-teal-accent">{t("app.login.forgotPassword")}</button>}
              </div>
