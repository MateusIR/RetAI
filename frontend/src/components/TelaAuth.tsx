  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Calcula o timestamp exato de expiração (agora + segundos retornados pelo servidor)
        const expiresAt = Date.now() + data.expires_in * 1000;

        onLogin(data.access_token, data.user, expiresAt);
      } else {
        ...
        if (!res.ok) {
          const errData = await res.json().catch(() => ({}));
          throw new Error(errData.detail || "Erro ao registrar médico");
        }
        setSenha("");
      ...
      <div className="card w-full max-w-md p-8 animate-fade-in shadow-2xl border-surface-4">
        {registrationSuccess ? (
          <div className="flex flex-col items-center justify-center py-6 animate-slide-up">
            ...
            <h2 className="font-display text-2xl font-bold text-white mb-2 text-center">Conta criada!</h2>
            <p className="text-slate-400 text-sm mb-8 text-center px-4 leading-relaxed">
              Dr(a).{" "}
              <span className="text-slate-200 font-medium">{nome.split(" ")[0]}</span>
          ...
          <>
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-slate-500 mt-1">
                {isLogin ? "Faça login" : "Crie sua conta médica"}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="animate-slide-up space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Dr. Nome" maxLength={100}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        required type="text" className="input-field font-mono text-sm"
                        placeholder="123456-UF" maxLength={15}
                        value={crm} onChange={(e) => setCrm(formatCrm(e.target.value))}
                      />
                    </div>
                  </div>
                </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="dr@clinica.com" maxLength={255}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    required type={showPassword ? "text" : "password"} className="input-field pr-10"
              ...
              >
                {loading ? (
                  <span className="flex items-center justify-center gap-2">Aguarde...</span>
