...
export default function App() {
  const [token, setToken] = useState<string | null>(
    localStorage.getItem("reta_auth_token"),
  );
  const [user, setUser] = useState<{
    id: number;
    nome: string;
    is_superadmin: boolean;
  } | null>(JSON.parse(localStorage.getItem("reta_user") || "null"));
  ...
  if (!token) {
    return (
      <TelaAuth
        onLogin={(tok, usr) => {
          localStorage.setItem("reta_auth_token", tok);
          localStorage.setItem("reta_user", JSON.stringify(usr));
          setToken(tok);
          setUser(usr);
        }}
      />
  ...
  return (
    <div className="min-h-screen bg-surface-0 flex flex-col">
            ...
            {menuOpen && (
              <div className="absolute top-10 right-0 w-48 bg-surface-2 border border-surface-4 rounded-xl shadow-2xl py-2 z-50">
                ...
                >
                  Usuários{" "}
                  {user?.is_superadmin && (
                    <span className="text-accent-glow ml-1 font-bold">
                      (Admin)
                    </span>
                  )}
                </button>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-8">
        {tab === "lista" && (
          <TelaLista refreshKey={refreshKey} currentUser={user} />
        )}
        {tab === "novo" && (
          <TelaNovo
            onSuccess={() => {
      ...
      {modalUsuarios && (
        <div
          ...
          onClick={(e) =>
            e.target === e.currentTarget && setModalUsuarios(false)
          }
        >
          <div className="card w-full max-w-2xl p-6 animate-slide-up">
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
              {listaUsers.map((u) => (
                <div
                  key={u.id}
                  className="p-3 bg-surface-3 rounded-lg border border-surface-4"
                >
                  {editUserId === u.id ? (
                    <div className="flex gap-2">
                      ...
                      <input
                        ...
                        value={editForm.crm}
                        onChange={(e) =>
                          setEditForm({ ...editForm, crm: e.target.value })
                        }
                        placeholder="CRM"
                      />
                      <input
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="text-sm font-medium text-white">
                          ...
                          {u.is_superadmin && (
                            <span className="text-[10px] bg-accent/20 text-accent-glow px-2 py-0.5 rounded ml-2 border border-accent/30">
                              ADMIN
                            </span>
                          )}
                        </p>
        ...
        >
          <div className="card w-full max-w-md p-6 animate-slide-up">
            ...
            <div className="text-sm text-slate-400 space-y-4">
              <p>
                O <strong>RetAI</strong> é uma ferramenta de triagem clínica
                baseada em inteligência artificial desenvolvida para auxiliar
                médicos na detecção de patologias oftalmológicas através de
                imagens de fundoscopia e OCT.
              </p>
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
                ...
                <p className="text-xs text-amber-500/80 leading-relaxed">
                  Os resultados apresentados por esta plataforma são
                  estritamente sugestivos e não substituem, em hipótese alguma,
                  a avaliação clínica, o laudo ou o julgamento profissional de
                  um médico qualificado.
                </p>
              </div>
              <p>
                Os dados processados são isolados por conta médica, mantendo a
                privacidade do paciente (LGPD/HIPAA).
              </p>
            </div>
            <button
              onClick={() => setModalSobre(false)}
