...
export default function App() {
  ...
  const showAlert = (title: string, message: string, type: DialogState['type'] = 'info'): Promise<void> =>
    new Promise(resolve =>
      setDialog({ title, message, type, confirmLabel: t('app.ok'), onConfirm: () => { setDialog(null); resolve() } })
    )

  const showConfirm = (
    title: string, message: string,
    type: DialogState['type'] = 'warning',
    confirmLabel = t('app.confirm'), cancelLabel = t('app.cancel')
  ): Promise<boolean> =>
    new Promise(resolve =>
      setDialog({
  ...
  const handleLogout = async () => {
    if (isProcessing) {
      const confirmed = await showConfirm(
        ...
        t('app.logoutWarningProcessing'),
        'warning', t('app.exitConfirm'), t('app.cancel')
      )
      if (!confirmed) return
    }
    logout()
          ...
          <div className="flex items-center gap-3 relative">
            ...
            <div className="flex items-center gap-1 bg-surface-2 rounded-lg border border-surface-4 p-0.5">
              <button
                ...
                className={`px-2 py-1 text-xs font-medium rounded ${i18n.language === 'pt-BR' ? 'bg-accent text-white' : 'text-slate-400 hover:text-white'}`}
              >
                PT
              </button>
              <button
                onClick={() => changeLanguage('en')}
                className={`px-2 py-1 text-xs font-medium rounded ${i18n.language === 'en' ? 'bg-accent text-white' : 'text-slate-400 hover:text-white'}`}
              >
                EN
              </button>
            </div>
      ...
      {modalUsuarios && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && setModalUsuarios(false)}>
          <div className="card w-full max-w-3xl p-6 animate-slide-up">
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="text-sm font-medium text-white flex items-center gap-1.5 flex-wrap">
                          ...
                          {u.is_superadmin && (
                            <span className="text-[10px] bg-accent/20 text-accent-glow px-2 py-0.5 rounded border border-accent/30">{t('app.adminBadge')}</span>
                          )}
                          {u.solicitou_reset && (
                            <span className="text-[10px] bg-orange-500/20 text-orange-400 px-2 py-0.5 rounded border border-orange-500/30 animate-pulse">{t('app.solicitedReset')}</span>
                          )}
                        </p>
            ...
            <button onClick={() => setModalUsuarios(false)} className="btn-ghost w-full mt-4">{t('app.close')}</button>
          </div>
        </div>
      )}
