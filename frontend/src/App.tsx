...
export default function App() {
  ...
  const [user, setUser] = useState<{id:number, nome:string, is_superadmin:boolean} | null>(JSON.parse(localStorage.getItem('reta_user') || 'null'))
  ...
  if (!token) {
    return <TelaAuth onLogin={(tok, usr) => { 
      localStorage.setItem('reta_auth_token', tok); localStorage.setItem('reta_user', JSON.stringify(usr)); 
      ...
      <header className="border-b border-surface-4 bg-surface-1/80 backdrop-blur-md sticky top-0 z-40 print:hidden">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 relative">
            ...
            <button onClick={handleLogout} className="text-sm font-medium text-slate-400 hover:text-red-400 transition-colors flex items-center gap-1.5" title="Sair do sistema">
              Sair
            </button>

            {/* Menu 3 Pontos */}
            <button onClick={() => setMenuOpen(!menuOpen)} className="p-1 rounded bg-surface-2 border border-surface-4 text-slate-400 hover:text-white transition-colors">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex gap-2">
                        {(user?.is_superadmin || u.id === user?.id) && (
                          <button onClick={() => {setEditUserId(u.id); setEditForm({nome: u.nome, crm: u.crm, email: u.email})}} className="text-xs text-blue-400 hover:bg-blue-400/10 px-2 py-1 rounded transition-colors">Editar</button>
                        )}
                        {user?.is_superadmin && !u.is_superadmin && (
                          <button onClick={()=>handleActionUser(u.id, 'prom')} className="text-xs text-emerald-400 hover:bg-emerald-400/10 px-2 py-1 rounded transition-colors">Tornar Admin</button>
                        )}
                        {(user?.is_superadmin || u.id === user?.id) && (
                          <button onClick={()=>handleActionUser(u.id, 'del')} className="text-xs text-red-400 hover:bg-red-400/10 px-2 py-1 rounded transition-colors">Excluir</button>
                        )}
                      </div>
                    </div>
                  )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={(e)=> e.target===e.currentTarget && setModalSobre(false)}>
          <div className="card w-full max-w-md p-6 animate-slide-up">
            ...
            <div className="text-sm text-slate-400 space-y-4">
              <p>O <strong>RetAI</strong> é uma ferramenta de triagem clínica baseada em inteligência artificial desenvolvida para auxiliar médicos na detecção de patologias oftalmológicas através de imagens de fundoscopia e OCT.</p>
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
                ...
                <p className="text-xs text-amber-500/80 leading-relaxed">Os resultados apresentados por esta plataforma são estritamente sugestivos e não substituem, em hipótese alguma, a avaliação clínica, o laudo ou o julgamento profissional de um médico qualificado.</p>
              </div>
