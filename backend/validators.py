...
def validar_correspondencia_nome(nome_input: str, nome_cfm: str) -> bool:
    ...
    if not tokens_input or not tokens_cfm: return False
...
def _parse_crm(crm: str, lang: str = "pt_BR") -> tuple[str, str]:
    ...
    if not m: m = re.match(r"^(\d+)([A-Z]{2})$", crm)
    if not m:
        raise HTTPException(status_code=422, detail=t("Formato de CRM inválido. Use: número seguido da UF (ex: 123456-SP, 123456/SP ou 123456 SP).", lang))
    numero, uf = m.group(1), m.group(2)
    if uf not in _UFS_VALIDAS:
        raise HTTPException(status_code=422, detail=t("UF '{}' inválida no CRM. Use a sigla do estado (ex: SP, RJ, MG).", lang).format(uf))
    return numero, uf

async def _consultar_crm_com_chave(numero: str, uf: str, chave: str, lang: str) -> dict | None:
    params = {"tipo": "crm", "uf": uf, "q": numero, "chave": chave, "destino": "json"}
    try:
...
async def validar_crm(crm: str, lang: str = "pt_BR") -> dict:
    ...
    if cache_key in _CRM_CACHE:
        ...
        if data is None:
            raise HTTPException(
                503,
                detail=t("Limite de consultas de CRM atingido em todas as chaves disponíveis. \nTente novamente amanhã ou contate o administrador. \nEm caso de urgencia, crie uma conta não verificada momentaneamente até o limite ser renovado.", lang)
            )
        _CRM_CACHE[cache_key] = data
        if len(_CRM_CACHE) > _CRM_CACHE_MAX_SIZE:
            _CRM_CACHE.popitem(last=False)

    total = int(data.get("total", 0))
    items = data.get("item") or []

    if not total or not items:
        raise HTTPException(404, detail=t("CRM {} não encontrado. Verifique o número e o estado.", lang).format(f"{numero}/{uf}"))
    ...
    if any(s in situacao for s in situacoes_invalidas):
        raise HTTPException(
            422,
            detail=t("CRM {} está com situação '{}' no cadastro e não pode ser usado para cadastro.", lang).format(f"{numero}/{uf}", situacao)
        )
